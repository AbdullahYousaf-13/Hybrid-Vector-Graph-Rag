...
| Context truncation (16000 chars) | `rag/vector_rag.py` | caps how much chunk text gets sent to the LLM; sized to fit all `k=6` chunks of up to 2000 chars (8000 used to cut off the last ~third), while still bounding datasets with much larger chunks |
