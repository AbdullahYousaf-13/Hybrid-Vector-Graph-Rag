...
| --- | --------------------------------------------------- | ----------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0   | CSV -> per-book JSON (one-off, not in `prep.ipynb`) | `KG/csv_to_json.py`                                               | Python stdlib `csv`/`json`                                                                     | takes the CSV path as an argument, groups rows by `book` then `chapter` (column names configurable) in file order, joins each section's rows into one text blob                                                                                                                                                                   |
...
def query_vector_rag(question, ...):
    ...
    return {"answer": result.strip(), "chunks": [c.page_content for c in chunks]}
```
...
| 5   | Format                          | `response["result"].strip()`                                            | plain Python                                                              | returned as-is so the model's line breaks survive                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
def generate_cypher_query(question, graph):
    ...
    names  = _entity_name_catalog(graph)                          # 2  real Book names
    ...
    return {"answer": response["result"].strip(), "cypher_query": cypher}  # 5
```
...
| 2   | Handle partial failure | plain Python                                            | if only one succeeded, return it directly (prefixed `"(<other> unavailable — ...)"`) and skip synthesis entirely — no need to spend a 3rd quota request reconciling one real answer against nothing; same when one side answered "I don't know" |                                                                                                                                                                                                                 |
...
| 4   | Format                 | `result.strip()`                                       | plain Python                                                                                                                                                                                        | returned as-is so the model's line breaks survive; same as the other two paths                                                                                                                                                                      |


`query_vector_rag` and `generate_cypher_query` each already increment the
shared Neo4j quota counter once internally; the synthesis step
...
def query_hybrid_rag(question, graph, ...):
    ...
    try:
        ...
        answer = result.strip()                                   # 4
    except Exception:
        answer = f"(Couldn't merge the two answers right now, so both are shown)\n\n## From the book text\n\n{vector_result['answer']}\n\n## From the knowledge graph\n\n{graph_result['answer']}"
